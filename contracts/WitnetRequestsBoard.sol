...
contract WitnetRequestsBoard is WitnetRequestsBoardInterface {
  ...
  modifier validDrOutputHash(uint256 _id) {
    require(requests[_id].drOutputHash == uint256(sha256(Request(requests[_id].requestAddress).bytecode())), "The dr has been manipulated and the bytecode has changed");
    _;
  }
    ...
    absMember(msg.sender)
 {
  ...
  function dataRequestCanBeClaimed(DataRequest memory _request) private view returns (bool) {
    return
      ...
      _request.drOutputHash == uint256(sha256(Request(_request.requestAddress).bytecode()));
  }
